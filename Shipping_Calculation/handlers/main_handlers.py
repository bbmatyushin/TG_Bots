...
async def query_derival_city(message: types.Message, state: FSMContext):
    ...
    if ut.check_cites_on_pop_list(message.text):  # проверяем есть ли город в популярных...
        pass
    else:  # если нет то добавляем туда
        ut.save_popular_cites(message.text)
    ...
    if check_derival_city:
        if len(check_derival_city) == 1:
            ...
            try:
                ...
                await message.reply(text=f"Найдено лишком много вариантов для *{message.text}*.\n"
                                         f"Попробуйте ввести больше первых букв названия города.",
                                    parse_mode='Markdown')
    else:
        await message.reply(text=f"Город *{message.text}* не найден. Возможно опечатка.\n"
...
async def query_arrival_city(message: types.Message, state: FSMContext):
    ...
    if check_arrival_city:
        if len(check_arrival_city) == 1:
            ...
            await message.reply("Выберите способ расчета:",
