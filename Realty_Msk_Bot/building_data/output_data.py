    ...
    def get_output_result(self, data_flainfo: dict, data_mingkh: dict, url_mingkh: str):
        ...
        if isinstance(flatinfo_data, str):
            ...
            if flatinfo_data.get("Назначение:") or mingkh_data.get('Назначение:'):
                ...
                if re.search(r'[Жж]илой.*дом', s, flags=re.ASCII):
            ...
            elif re.search(r'\d+\b', url_f) or re.search(r'\d+\b', url_gkh):  # если url flatinfo заканчивается на цифры, значит какая-то инфа есть
                self.output_result = [f"🌐 <a href='{url_f}'>FlaInfo.ru</a> | "
                                      f"<a href='{url_gkh}'>ДОМ.МИНЖКХ</a>\n"
                                      f"🏗 <b>{addr_name_f}</b>\n"
                                      f"-------------------------------\n"
