...
class SelectQueries(CreateDataTables):
    ...
    def select_rarity(self, value_rarity='70', table='ton_diamonds') -> list:
    ...
    def select_target_rarity(self, value_rarity='70', table='ton_diamonds') -> list:
        if table == "ton_dns_domains":
            return [f"У этой коллекции нет значени я редкости и рейтинга."]
        else:
            with self.pg_create_conn() as conn:
...
class SelectResult(SelectQueries):
    ...
    def get_rarity_analytic(self, client_message='70', table='ton_diamonds'):
        """Выдача результата для анализа редкости в широком диапазоне - выборка до 30 шт"""
        try:
            data = self.select_rarity(value_rarity=client_message, table=table)
            try:
                ...
                if len(data[-1]) == 1:
                    ...
                    s1 = f"◗ <b>Мин.цена (1):</b>\n↳ {data[-1][0][0]:,} TON | <b>R:</b> {data[-1][0][1]:.2f}\n"
                    s2 = f"◗ <b>Мин.цена (2):</b>\n↳ {data[-1][1][0]:,} TON (+{(data[-1][1][0] / data[-1][0][0] - 1) * 100:.2f}%) | <b>R:</b> {data[-1][1][1]:.2f}\n"
                    s3 = f"◗ <b>Макс.цена:</b> {data[-1][-1][0]:,} TON | <b>R:</b> {data[-1][-1][1]:.2f}"
                    self.print_row = s1 + s2 + s3
                elif len(data[-1]) >= 4:
                    s1 = f"◗ <b>Мин.цена (1):</b>\n↳ {data[-1][0][0]:,} TON | <b>R:</b> {data[-1][0][1]:.2f}\n"
                    s2 = f"◗ <b>Мин.цена (2):</b>\n↳ {data[-1][1][0]:,} TON (+{(data[-1][1][0] / data[-1][0][0] - 1) * 100:.2f}%) | <b>R:</b> {data[-1][1][1]:.2f}\n"
                    s3 = f"◗ <b>Мин.цена (3):</b>\n↳ {data[-1][2][0]:,} TON (+{(data[-1][2][0] / data[-1][0][0] - 1) * 100:.2f}%) | <b>R:</b> {data[-1][2][1]:.2f}\n"
            ...
            self.output = [f"Бот ожидает в сообщении только цифры в формате 123.45 (￢_￢;)"]

        return ''.join(self.output)

    def get_target_rarity_analytic(self, client_message='70', table='ton_diamonds'):
        """Выдача результата для анализа редкости в узком диапазоне - выборка до 5 шт"""
        try:
            data = self.select_target_rarity(value_rarity=client_message, table=table)
            try:
                # data[-1] - это списко кортежей с ценой, редкостью и рейтингом предметов из выборки
                if len(data[-1]) == 1:
                    self.print_row = f"""◗ <b>Цена:</b> {data[-1][0][0]:,} TON | <b>R:</b> {data[-1][0][1]:.2f}"""
