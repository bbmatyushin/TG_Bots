...
class CreateDatabaseSpread:
    """Сoзданиe и наполнение таблиц для информации по спредам"""
...
class CreateDatabasePump:
    """Делаем отдельную БД для сбора инфы по пампу, чтобы не блокировать
    доступ во время парсинга к БД по спредам"""

    def __init__(self):
        self.base = sqlite3.connect(f"{dir_database}/pump_db")
        self.cursor = self.base.cursor()
        self.parser = MainParser()
    ...
    def insert_temp_table_all_data_coins(self):
        ...
        self.cursor.execute("DELETE FROM temp_all_data_coins")
